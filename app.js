...
const modernMobileCanvas = $('#modernMobileCanvas');
...
const historicMobileCanvas = $('#historicMobileCanvas');
...
// Mobile canvases paint only the pixels visible on screen. The browser never
// needs to create a 4K–10K transformed texture when a close stage is opened.
function drawMobileRaster(canvas, image, current, mapWidth, mapHeight) {
  if (!canvas?.getContext || !image?.complete || !image.naturalWidth || !image.naturalHeight) return;
  const context = canvas.getContext('2d', { alpha: false });
  if (!context) return;
  const ratio = Math.min(devicePixelRatio || 1, 1.5);
  const pixelWidth = Math.max(1, Math.round(innerWidth * ratio));
  const pixelHeight = Math.max(1, Math.round(innerHeight * ratio));
  if (canvas.width !== pixelWidth) canvas.width = pixelWidth;
  if (canvas.height !== pixelHeight) canvas.height = pixelHeight;
  const box = mobileViewBox(current, mapWidth, mapHeight);
  const sx = box.left * image.naturalWidth / mapWidth;
  const sy = box.top * image.naturalHeight / mapHeight;
  const sw = box.width * image.naturalWidth / mapWidth;
  const sh = box.height * image.naturalHeight / mapHeight;
  context.setTransform(ratio, 0, 0, ratio, 0, 0);
  context.fillStyle = '#d7cdb7';
  context.fillRect(0, 0, innerWidth, innerHeight);
  context.drawImage(image, sx, sy, sw, sh, 0, 0, innerWidth, innerHeight);
}
...
function paintCameras() {
  ...
  if (historicCameraDirty) {
    ...
    if (mobileMapMode.matches) {
      ...
      drawMobileRaster(historicMobileCanvas, historicMapRaster, view, MAP_WIDTH, MAP_HEIGHT);
    } else {
  ...
  if (modernCameraDirty) {
    ...
    if (mobileMapMode.matches) {
      ...
      drawMobileRaster(modernMobileCanvas, modernMapRaster, modernView, MODERN_MAP_WIDTH, MODERN_MAP_HEIGHT);
    } else {
...
function positionCoverMap() {
  ...
  const renderScale = mobileMapMode.matches ? MOBILE_HISTORIC_WIDTH / MAP_WIDTH : 1;
  const transform = `translate(${-x * zoom}px, ${-y * zoom}px) scale(${zoom / renderScale})`;
