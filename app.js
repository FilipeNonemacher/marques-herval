...
const modernMapRaster = $('#modernMapRaster');
const modernDesktopSource = $('#modernDesktopSource');
const modernOverlay = $('.modern-map-canvas');
const mapWorld = $('.map-world');
const historicRasterWorld = $('#historicRasterWorld');
const historicMapRaster = $('.historic-map');
...
// On touch devices the map stays inside a compositor-friendly size. This
// avoids the oversized layers that make Safari and lower-memory Android
// browsers drop the raster or terminate the page during close-up stages.
const MOBILE_MAX_ZOOM = .38;
...
function paintMobileMap(surface, overlay, current, width, height) {
  const box = mobileViewBox(current, width, height);
  surface.style.backgroundSize = `${width * current.zoom}px ${height * current.zoom}px`;
  surface.style.backgroundPosition = `${innerWidth / 2 - current.x * current.zoom}px ${innerHeight / 2 - current.y * current.zoom}px`;
  overlay.setAttribute('viewBox', `${box.left} ${box.top} ${box.width} ${box.height}`);
}
...
function paintCameras() {
  ...
  if (historicCameraDirty) {
    ...
    if (mobileMapMode.matches) {
      mapWorld.style.transform = 'none';
      historicRasterWorld.style.transform = 'none';
      paintMobileMap(historicRasterWorld, historicOverlay, view, MAP_WIDTH, MAP_HEIGHT);
    } else {
      ...
      historicOverlay.setAttribute('viewBox', `0 0 ${MAP_WIDTH} ${MAP_HEIGHT}`);
    }
  ...
  if (modernCameraDirty) {
    ...
    if (mobileMapMode.matches) {
      modernWorld.style.transform = 'none';
      paintMobileMap(modernWorld, modernOverlay, modernView, MODERN_MAP_WIDTH, MODERN_MAP_HEIGHT);
    } else {
      ...
      modernOverlay.setAttribute('viewBox', `0 0 ${MODERN_MAP_WIDTH} ${MODERN_MAP_HEIGHT}`);
    }
...
function ensureModernMapLoaded() {
  if (modernMapLoaded) return;
  modernMapLoaded = true;
  // Mobile is the safe default. Only a wide device with a precise pointer is
  // allowed to select the large cartographic source.
  modernDesktopSource.srcset = 'public/assets/mapa-atual-br-ers.png';
  modernMapRaster.src = 'public/assets/mapa-atual-br-ers-phone.png';
  if (mobileMapMode.matches) modernWorld.style.backgroundImage = "url('public/assets/mapa-atual-br-ers-phone.png')";
  if (modernMapRaster.decode) modernMapRaster.decode().catch(() => {});
}

historicMapRaster.addEventListener('load', () => {
  if (!mobileMapMode.matches) return;
  historicRasterWorld.style.backgroundImage = `url("${historicMapRaster.currentSrc || historicMapRaster.src}")`;
});
modernMapRaster.addEventListener('load', () => {
  if (!mobileMapMode.matches) return;
  modernWorld.style.backgroundImage = `url("${modernMapRaster.currentSrc || modernMapRaster.src}")`;
});

// If a partial GitHub Pages upload omitted one responsive asset, retry with the
// other cartographic source instead of leaving an opaque black map.
function installMapFallbacks() {
  document.querySelectorAll('[data-mobile-map]').forEach((image) => {
    const recover = () => {
      if (image.dataset.mapFallbackUsed) return;
      const failed = image.currentSrc || image.getAttribute('src') || '';
      const source = image.parentElement?.querySelector('source');
      source?.removeAttribute('srcset');
      image.dataset.mapFallbackUsed = 'true';
      image.src = failed.includes(image.dataset.mobileMap) ? image.dataset.desktopMap : image.dataset.mobileMap;
    };
    image.addEventListener('error', recover);
    const requested = image.currentSrc || image.getAttribute('src') || image.parentElement?.querySelector('source')?.getAttribute('srcset');
    if (requested && image.complete && !image.naturalWidth) recover();
  });
}
...
function positionCoverMap() {
  // Introductory pages do not need a live 8K camera on phones. A viewport-sized
  // crop avoids Safari/Android allocating a texture larger than their GPU limit.
  if (mobileMapMode.matches) {
...
// Every lower caption remains on one line on phones. The available width and
// the actual bundled typeface are measured, so long historical descriptions
// shrink only as much as each stage really needs.
function fitMobileStageTitle(title) {
  title.style.fontSize = '';
  if (innerWidth > 760) return;
  let size = title.closest?.('.title-card')?.classList.contains('is-attempt') ? 10 : 13;
  title.style.fontSize = `${size}px`;
  while (size > 4.5 && title.scrollWidth > title.clientWidth) {
    size -= .25;
    title.style.fontSize = `${size}px`;
  }
}
...
addEventListener('resize', () => {
  ...
  if (modernActive) {
    ...
    if (!chromeOnlyResize && !MODERN_AWAITING_MARKINGS) renderModern({ animate: false });
