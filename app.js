...
const modernMobileSource = $('#modernMobileSource');
...
const MOBILE_HISTORIC_WIDTH = 2048;
const MOBILE_MODERN_WIDTH = 2048;
const stages = [
...
function paintCameras() {
  ...
  if (historicCameraDirty) {
    ...
    const transform = `translate(${-view.x * view.zoom}px, ${-view.y * view.zoom}px) scale(${view.zoom / renderScale})`;
  ...
  if (modernCameraDirty) {
    ...
    const renderScale = mobileMapMode.matches ? MOBILE_MODERN_WIDTH / MODERN_MAP_WIDTH : 1;
    const transform = `translate(${-modernView.x * modernView.zoom}px, ${-modernView.y * modernView.zoom}px) scale(${modernView.zoom / renderScale})`;
...
function ensureModernMapLoaded() {
  ...
  // The source is deliberately absent from the initial HTML. A phone therefore
  // never keeps both enormous source maps in memory while entering history.
  modernMobileSource.srcset = 'public/assets/mapa-atual-br-ers-phone.png';
  modernMapRaster.src = 'public/assets/mapa-atual-br-ers.png';
